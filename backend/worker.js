 ...
 *
 * Worker secrets to set in Cloudflare Dashboard → Worker → Settings → Variables:
...
const SEARCHES = {
  wigs: [
    '4c kinky curly lace front wig','kinky coily lace wig human hair',
    'afro kinky curly human hair wig','deep wave lace front wig black women',
    'water wave human hair lace wig','body wave lace wig natural black',
    'tight curly afro lace wig','glueless lace wig kinky curly',
    'HD lace wig kinky curly human hair','loose deep wave lace front wig',
    'short bob lace front wig human hair','headband wig kinky curly human hair',
    'straight lace front wig human hair','v part wig human hair curly',
    'coily afro human hair lace wig',
  ],
  bundles: [
    '4c kinky curly human hair bundles','kinky coily human hair weave bundles',
    'afro kinky bulk human hair braiding','deep wave human hair bundles black women',
    'water wave Brazilian hair bundles','body wave human hair bundles weave',
    'tight curly human hair weft bundles','natural afro kinky hair bundles',
    'loose deep wave hair bundles weft','straight human hair bundles weave',
    'Peruvian kinky curly hair bundles','Malaysian deep wave hair bundles',
    'raw Indian kinky curly hair bundles','kinky straight human hair bundles',
    'natural wave human hair bundles weft',
  ],
  lashes: [
    '3D mink false eyelashes dramatic volume','mink lashes wispy natural look',
    'false eyelashes fluffy faux mink','cluster lash extensions DIY kit',
    'individual lash fans professional','wispy dramatic false eyelashes set',
    'magnetic eyelashes natural mink','human hair false lashes natural',
    'mega volume mink lashes set','cat eye false eyelashes mink',
    'natural wispies false lashes','reusable mink false eyelashes',
    'lash extension kit professional set','demi wispies false eyelashes',
    'handmade mink lashes dramatic',
  ],
  nails: [
    'press on nails coffin long set','false nails almond stiletto acrylic',
    'rhinestone press on nails coffin','ombre glitter press on nails set',
    'gel press on nails long coffin','black press on nails coffin set',
    'pink ombre press on nails almond','french tip press on nails set',
    'acrylic nail tips full cover set','nail art press on decoration set',
    'marble press on nails coffin','holographic glitter press on nails',
    'butterfly nail art press on set','matte black press on nails stiletto',
    'short square press on nails set',
  ],
  accessories: [
    'wig cap hairnet adjustable','lace wig glue adhesive waterproof',
    'wig grip band non slip headband','lash glue eyelash adhesive waterproof',
    'lash applicator tweezers tool kit','nail glue false nail adhesive set',
    'nail art rhinestone decoration kit','nail file buffer manicure set',
    'wig stand mannequin head holder','lace wig install melting spray kit',
    'wig tape adhesive double sided','nail art brush decoration tool',
    'lash sealer coating mascara','wig comb pin tail styling',
...
function inRange(price, cat) { const r = PRICE_AFTER[cat]; return !r || (price >= r.min && price <= r.max); }
...
function toProduct(item, cat) {
  ...
  return {
    id: item.pid, name, category: detCat, price,
    cjCost: parseFloat(raw.toFixed(2)),
  ...
  try {
    await fetch('https://api.mailchannels.net/tx/v1/send', {
      ...
      body: JSON.stringify({
        ...
        from: { email: 'orders@clcbeauties.com', name: 'CLC Beauties' },
        subject: `💰 Order ${num} — Profit $${bd.profit.toFixed(2)}`,
...
export default {
  async fetch(req, env) {
    ...
    if (path === '/api/products/slider') {
      try {
        const tok = await getToken(CJ), seen = new Set(), products = [];
        const sliderKws = ['4c kinky curly lace front wig','afro kinky curly human hair wig','deep wave lace wig black women','body wave lace wig natural black','water wave human hair wig','glueless lace wig kinky curly','HD lace wig kinky human hair','tight curly afro lace wig'];
        for (const kw of sliderKws) {
          ...
          try {
            ...
            if (d.code !== 200) continue;
            for (const item of (d.data?.list || [])) {
              if (seen.has(item.pid)) continue; seen.add(item.pid);
    ...
    if (path.startsWith('/api/products/category/')) {
      ...
      try { const tok = await getToken(CJ); return ok({ success: true, category: cat, total: 0, products: await fetchCat(cat, tok, 80) }); }
      catch(e) { return ok({ error: e.message, products: [] }, 500); }
    }

    if (path === '/api/products/all') {
      try {
        const tok = await getToken(CJ), all = [];
    ...
    if (path === '/api/orders' && method === 'POST') {
      try {
        ...
        const tok = await getToken(CJ), num = `CLC-${Date.now()}`;
