...
// CJ Auth — token cached in-memory per Worker instance
let _tok = '', _exp = 0;
...
// Fetch category — 15 searches × up to 50 products each = up to 750 raw, filtered to max
...
// Profit email via MailChannels (free on Cloudflare Workers)
...
export default {
  async fetch(req, env) {
    ...
    // GET /health
    if (path === '/health') {
      return ok({ status: 'ok', store: 'www.CLCBeauties.com', worker: true, cj: !!CJ, stripe: !!SK, markup: '15%', version: '4.0', searches_per_cat: 15 });
    }

    // GET /api/auth-test
    if (path === '/api/auth-test') {
      try {
        const t = await getToken(CJ);
        return ok({ success: true, message: 'CJ connected!', preview: t.substring(0, 10) + '...' });
      } catch (e) {
        return ok({ success: false, error: e.message }, 401);
      }
    }
    ...
    if (path === '/api/products/slider') {
      try {
        ...
        return ok({ error: e.message, products: [] }, 500);
      }
    }
    ...
    // GET /api/products?keyword= — free-form search
    ...
    if (path === '/api/orders' && method === 'POST') {
      try {
        ...
        const bd = {
          items:    items.map(i => ({ name: i.name, qty: i.qty, cjCost: i.cjCost, price: i.price, profit: i.price - i.cjCost })),
          cjTotal:  parseFloat(cjTotal.toFixed(2)),
          charged:  parseFloat(charged.toFixed(2)),
          profit:   parseFloat((charged - cjTotal).toFixed(2)),
        };
